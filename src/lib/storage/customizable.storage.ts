...
export const DEFAULT_CUSTOMIZABLE_STATE: CustomizableState = {
  ...
  hideFromScreenCapture: { isEnabled: true },
};
...
export const getCustomizableState = (): CustomizableState => {
  try {
    ...
    return {
      ...
      hideFromScreenCapture: {
        isEnabled:
          parsedState.hideFromScreenCapture?.isEnabled ??
          DEFAULT_CUSTOMIZABLE_STATE.hideFromScreenCapture.isEnabled,
      },
    };
  } catch (error) {
...
/**
 * Update whether the overlay is hidden from screen share and recording
 */
export const updateHideFromScreenCapture = (
  isEnabled: boolean
): CustomizableState => {
  const currentState = getCustomizableState();
  const newState = { ...currentState, hideFromScreenCapture: { isEnabled } };
  setCustomizableState(newState);
  return newState;
};
